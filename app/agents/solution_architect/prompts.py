...
NODE_PROMPTS: dict[str, str] = {
    ...
    "architect-explore": (
        ...
        "Cloud selection is mandatory: use discovery.cloud / connected_clouds and the "
        "mapped GitHub repo. If docs are empty, the connected cloud account(s) and repo "
        "are the source of truth. Never propose Azure services when the project is on AWS "
        "(or the reverse) unless both clouds are connected and the evidence supports it.\n"
    ...
    "architect-design": (
        ...
        "Tier={{tier}} mode={{mode}}.\n"
        "Target cloud MUST match discovery.cloud (aws / azure / gcp). Name native "
        "services for that cloud only (e.g. ECS/RDS/Secrets Manager on AWS; "
        "Container Apps/PostgreSQL Flexible Server/Key Vault on Azure).\n"
    ...
    "architect-verify": (
        ...
        "risk_class MUST be exactly one of read_diagnose|reversible_change|"
        "config_code_change|irreversible_high_blast. blast_radius MUST be exactly "
        "low|medium|high — never a sentence or resource name.\n"
