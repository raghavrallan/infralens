...
class ComplianceMapperSkill(Skill):
    ...
    system_prompt = (
        ...
        "EVIDENCE HANDLING:\n"
        "- Controls is a read-only evidence bundle gathered from the user's connected "
        "cloud account, repository inventory, dependency/security configuration, and "
        "CI workflows. Base the mapping on those concrete artifacts.\n"
        "- A framework is always supplied by the orchestrator. Use it directly; if "
        "the evidence is incomplete, mark the affected criterion Partial or Gap and "
        "name the missing artifact. Do not ask the user to paste controls when the "
        "bundle is present.\n"
        "- Do not treat a provider error or absent artifact as proof that a control "
        "is absent. Label it as unable to verify and distinguish that from a confirmed "
        "gap. Never invent implementation evidence.\n\n"
        "OUTPUT (Markdown):\n"
