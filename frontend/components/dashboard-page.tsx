  ...
  return <Shell subtitle="Intelligence Layer" scroll>
    <main className="dash">
      <div className="dash-head"><div><h2 className="page-title">DevOps Intelligence</h2><p className="page-sub">Skills run unattended in a queue. Every finding is gated by action class and blast radius — you gate entry into risk, never the escape.</p></div>
        <div className="dash-head-controls"><label className="control"><span>Project</span><ThemedSelect className="project-select" value={projectId} ariaLabel="Project" onChange={selectProject} options={projects.map((project) => ({ value: project.id, label: project.name }))} /></label><span className="updated-note">{updated ? `Updated ${timeAgo(updated.toISOString())}` : "Loading"}</span><button className="ghost" onClick={() => void loadData()}>Refresh</button><button className="primary" onClick={() => setWorkflowModal(null)}>+ Workflow</button></div>
      </div>
...
function WorkflowModal({ existing, catalog, projectId, onClose, onSaved }: { existing: Workflow | null; catalog: Catalog; projectId: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal eyebrow={existing ? "Edit workflow" : "New workflow"} title={existing ? "Update this workflow" : "Automate diagnose skills"} description="Only read-only skills can run unattended. Pick the skills, an optional schedule, and the environment used to gate findings." onClose={onClose}><form className="modal-body" onSubmit={(event) => void save(event)}><label className="modal-label"><span>Name</span><input value={draft.name} onChange={(event) => update("name", event.target.value)} placeholder="Nightly Posture Sweep" /></label><label className="modal-label"><span>Objective</span><input value={draft.objective} onChange={(event) => update("objective", event.target.value)} placeholder="Review posture and drift" /></label><label className="modal-label"><span>Module</span><ThemedSelect className="modal-select" value={draft.module} ariaLabel="Module" onChange={(value) => update("module", value)} options={catalog.modules.map((item) => ({ value: item.key, label: item.label }))} /></label><label className="modal-label"><span>Environment</span><ThemedSelect className="modal-select" value={draft.environment} ariaLabel="Environment" onChange={(value) => update("environment", value as WorkflowDraft["environment"])} options={[{ value: "prod", label: "Production" }, { value: "staging", label: "Staging" }, { value: "dev", label: "Dev" }]} /></label><label className="modal-label"><span>Schedule (cron, optional)</span><input value={draft.schedule_cron} onChange={(event) => update("schedule_cron", event.target.value)} placeholder="0 2 * * *" /></label><div className="modal-label"><span>Skills</span><div className="modal-skill-grid">{catalog.skills.map((skill) => <label className="modal-skill" key={skill.name}><input type="checkbox" checked={draft.skills.includes(skill.name)} onChange={(event) => update("skills", event.target.checked ? [...draft.skills, skill.name] : draft.skills.filter((item) => item !== skill.name))} />{prettyName(skill.name)}</label>)}</div></div><div className="modal-actions"><button type="button" className="modal-btn ghost" onClick={onClose}>Cancel</button><button type="submit" className="modal-btn primary">{existing ? "Save" : "Create"}</button></div></form></Modal>;
}
