...
export function DashboardPage() {
  ...
  return <Shell subtitle="Intelligence Layer" scroll loading={loading}>
    ...
    {workflowModal !== undefined && projectId && <WorkflowModal existing={workflowModal} catalog={catalog} projectId={projectId} onClose={() => setWorkflowModal(undefined)} onSaved={() => { setWorkflowModal(undefined); void loadData(); }} />}
...
function WorkflowModal({ existing, catalog, projectId, onClose, onSaved }: { existing: Workflow | null; catalog: Catalog; projectId: string; onClose: () => void; onSaved: () => void }) {
  ...
  const initialModuleSkills = catalog.modules.find((item) => item.key === initialModule)?.skills || [];
  const [draft, setDraft] = useState<WorkflowDraft>({
    ...
    skills: initialModuleSkills.length ? [...initialModuleSkills] : existing?.skills || [],
  });
  const compatibleSkills = useMemo(
    () => new Set(catalog.modules.find((item) => item.key === draft.module)?.skills || []),
  ...
  const selectModule = (value: string) => {
    const skills = catalog.modules.find((item) => item.key === value)?.skills || [];
    setDraft((current) => ({ ...current, module: value, skills: [...skills] }));
  };
  const save = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!draft.skills.length) return;
    const options: RequestInit = { method: existing ? "PATCH" : "POST", body: JSON.stringify({ ...draft, enabled: existing?.enabled ?? true }) };
    const url = existing ? `/api/workflows/${existing.id}` : `/api/workflows?project_id=${encodeURIComponent(projectId)}`;
    await api(url, options);
    onSaved();
  };
  return <Modal eyebrow={existing ? "Edit workflow" : "New workflow"} title={existing ? "Update this workflow" : "Automate diagnose skills"} description="Only read-only skills can run unattended. Pick the skills, an optional schedule, and the environment used to gate findings." onClose={onClose}><form className="modal-body" onSubmit={(event) => void save(event)}><label className="modal-label"><span>Name</span><input value={draft.name} onChange={(event) => update("name", event.target.value)} placeholder="Nightly Posture Sweep" /></label><label className="modal-label"><span>Objective</span><input value={draft.objective} onChange={(event) => update("objective", event.target.value)} placeholder="Review posture and drift" /></label><label className="modal-label"><span>Module</span><ThemedSelect className="modal-select" value={draft.module} ariaLabel="Module" onChange={selectModule} options={catalog.modules.map((item) => ({ value: item.key, label: item.label }))} /></label><label className="modal-label"><span>Environment</span><ThemedSelect className="modal-select" value={draft.environment} ariaLabel="Environment" onChange={(value) => update("environment", value as WorkflowDraft["environment"])} options={[{ value: "prod", label: "Production" }, { value: "staging", label: "Staging" }, { value: "dev", label: "Dev" }]} /></label><label className="modal-label"><span>Schedule (cron, optional)</span><input value={draft.schedule_cron} onChange={(event) => update("schedule_cron", event.target.value)} placeholder="0 2 * * *" /></label><div className="modal-label"><span>Skills</span><div className="modal-skill-grid">{catalog.skills.map((skill) => { const compatible = compatibleSkills.has(skill.name); return <label className={`modal-skill${compatible ? "" : " disabled"}`} key={skill.name}><input type="checkbox" checked={compatible && draft.skills.includes(skill.name)} disabled={!compatible} onChange={(event) => update("skills", event.target.checked ? [...draft.skills, skill.name] : draft.skills.filter((item) => item !== skill.name))} />{prettyName(skill.name)}</label>; })}</div></div><div className="modal-actions"><button type="button" className="modal-btn ghost" onClick={onClose}>Cancel</button><button type="submit" className="modal-btn primary">{existing ? "Save" : "Create"}</button></div></form></Modal>;
}
